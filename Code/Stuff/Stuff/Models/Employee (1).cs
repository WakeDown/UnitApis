...
namespace Stuff.Models
{
    public class Employee : DbModel
    {
        ...
        public static IEnumerable<Employee> GetList(int? idCity = null)
        {
            Uri uri = new Uri(String.Format("{0}/Employee/GetList?idCity={1}", OdataServiceUri, idCity));
            string jsonString = GetJson(uri);
        ...
        {
            Uri uri = new Uri(String.Format("{0}/Employee/GetDepartmentDirector", OdataServiceUri));
            string jsonString = GetJson(uri);
            Employee emp = JsonConvert.DeserializeObject<Employee>(jsonString);
            FillSelf(emp);
            return emp;
